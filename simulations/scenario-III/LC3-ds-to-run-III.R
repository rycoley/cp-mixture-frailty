...
#Accompanies LC3-ds-III.R and LC3-ds-source-III.R 
...
pm[,1]<-expit(cv[1]-as.vector(true.V%*%gamma))
pm[,2]<-expit(cv[2]-as.vector(true.V%*%gamma))-expit(cv[1]-as.vector(true.V%*%gamma))
pm[,3]<-1-expit(cv[2]-as.vector(true.V%*%gamma))
...
Tf<-quantile(Y,0.06) #0.06 #0.03 #cut
delta<-rep(0,n)
delta[Y<=Tf]<-1		
Y[Y>Tf]<-Tf
...
for(d in 1:2){gam.res[d,]<-c(median(gam.mat[,d]), cover(x=gam.mat[,d], true=gam.true[d]))}
gam.res[D,]<-c(median(gam.mat[,D]), cover(x=gam.mat[,D], true=gam.true[true.D]))
