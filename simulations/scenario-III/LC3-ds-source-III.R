...
#Accompanies LC3-ds-III.R and LC3-ds-to-run-III.R 
...
sti<-c(rep(1,100),rep(0,400), rep(1,80),rep(0,420), rep(1,60), rep(0,440)) #rbinom(n,1,0.2)
marr.co<-rep(0,n)
for(i in 1:n){if(age[i]==1){marr.co[i]<-rbinom(1,1,0.25)}
	else{if(age[i]==2){marr.co[i]<-rbinom(1,1,0.5)}
		else{marr.co[i]<-rbinom(1,1,0.75)}}}

true.V<-as.matrix(cbind(age==1, age==2, sti, marr.co))
true.D<-dim(true.V)[2]
V<-as.matrix(cbind(age==1, age==2, marr.co))
D<-dim(V)[2]
...
#	nu<-(1/n)*(t(as.matrix(cm%*%rhoh))%*%as.matrix(cm%*%eta))
...
MH.Ri<-function(R0, Vi, cv, gam, rho, Ni, eta, nu, Zi){ #give to fcn as R0=class.vec[i]==1
	U<-runif(1,0,1)
