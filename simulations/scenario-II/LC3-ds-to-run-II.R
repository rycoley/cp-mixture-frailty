...
#Accompanies LC3-ds-II.R and LC3-ds-source-II.R 
...
#this code is analogous to LC3-ds-to-run-I.R. Please see annotated version in this same folder on github for explanation of variable names, functions, etc.
...
for(j in 2:B){
	...
	cv<-slice.c(c0=cv, gam=gamhat, class.vec=class.vec, rhoh=rhohat, etah=etahat, eta.vec=eta.vec, Ns=Nhat, Zs=Zhat)

	gamhat<-slice.gam(gam0=gamhat, cv=cv, class.vec=class.vec, rhoh=rhohat, etah=etahat, eta.vec=eta.vec, Ns=Nhat, Zs=Zhat)
	...
	etahat<-slice.eta(eta0=etahat, Ns=Nhat, Zs=Zhat, rhoh=rhohat, class.vec=class.vec, pm=pm, cm=cm)
...
for(i in 1:n){
	...
	lam<-rgamma(1,shape=(sum(delta)+1), rate=(sum(Zhat*exp(X.mat*bet)*Y)))
...
#		if(j%in%seq(1,A,ke)){print(j)}
		...
		HD[k.num]<-get.yrep.surv(bet=bet, lam=lam, gamhat=gamhat, cv=cv, rhohat=rhohat, etahat=etahat, pdf.true=pdf.true, Tf=Tf, Tf.mo=Tf.mo)}
}
