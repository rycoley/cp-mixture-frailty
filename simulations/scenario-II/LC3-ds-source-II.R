...
##Simulation code for Scenario II: Noninformative priors and correctly specified model
#Accompanies LC3-ds-II.R and LC3-ds-to-run-II.R
...
#this code is analogous to LC3-ds-source-I.R. Please see annotated version in this same folder on github for explanation of variable names, functions, etc.
...
lik.c<-function(cv, k, gam, class.vec, rhoh, etah, eta.vec, Ns, Zs){
	...
	if(k==2){return(sum(log(expit(cv[2]-as.vector(V[class.vec==2,]%*%gam)) - expit(cv[1]-as.vector(V[class.vec==2,]%*%gam)))) + lik.nu+ sum(log(1-expit(cv[2]-as.vector(V[class.vec==3,]%*%gam))))  )}	
		}
	
slice.c<-function(c0, gam, class.vec, rhoh, etah, eta.vec, Ns, Zs){
	for(k in 1:2){
	if(k==1){min.c<- -5 
		max.c<- c0[2]-1e-10}
	...
	z<- lik.c(cv=c0, k=k, gam=gam, class.vec=class.vec, rhoh=rhoh, etah=etah, eta.vec=eta.vec, Ns=Ns, Zs=Zs) -rexp(1,1) 
	w<- 0.05
	m<- 10
	cL<-cR<-c.star<-c0
	...
	J<- floor(m*runif(1,0,1))
	K<- (m-1)-J
	while(cL[k]>min.c & lik.c(cv=cL, k=k, gam=gam, class.vec=class.vec, rhoh=rhoh, etah=etah, eta.vec=eta.vec, Ns=Ns, Zs=Zs) > z & J>0){cL[k]<-max(cL[k]-w, min.c)
		J<- J-1}
		cL[k]<-max(cL[k],min.c)
	while(cR[k]<max.c & lik.c(cv=cR, k=k, gam=gam, class.vec=class.vec, rhoh=rhoh, etah=etah, eta.vec=eta.vec, Ns=Ns, Zs=Zs) > z & K>0){cR[k]<- min(cR[k]+w, max.c)
		K<- K-1}
		cR[k]<-min(cR[k],max.c)
	c.star[k]<- runif(1,cL[k],cR[k])
	while(lik.c(cv=c.star, k=k, gam=gam, class.vec=class.vec, rhoh=rhoh, etah=etah, eta.vec=eta.vec, Ns=Ns, Zs=Zs) < z){
...
lik.gam<-function(gam, d, cv, class.vec, rhoh, etah, eta.vec, Ns, Zs){
	
	pm<-get.pm(cv=cv, gam=gam)
	nu<-sum(rhoh*etah*apply(pm,2,mean))
	lik.nu<-get.lik.nu(nuhat=nu, Ns=Ns, Zs=Zs, eta.vec=eta.vec)

	return(	sum(-as.vector(V[class.vec==1,]%*%gam)) - sum(log(1+exp(cv[1]-as.vector(V[class.vec==1,]%*%gam)))) + 	sum(log(expit(cv[2]-as.vector(V[class.vec==2,]%*%gam))-expit(cv[1]-as.vector(V[class.vec==2,]%*%gam)))) + sum(log(1-expit(cv[2]-as.vector(V[class.vec==3,]%*%gam)))) +lik.nu)}



slice.gam<-function(gam0, cv, class.vec, rhoh, etah, eta.vec, Ns, Zs){
	for(d in 1:D){
			min.gam<- -2.5
			max.gam<- 2.5
	z<-J<-K<-L<-R<-gam.star<-NULL
	z<- lik.gam(gam=gam0, d=d, cv=cv, class.vec=class.vec, rhoh=rhoh, etah=etah, eta.vec=eta.vec, Ns=Ns, Zs=Zs) - rexp(1,1)
	w<- 0.05
	m<- 10
	...
	J<- floor(m*runif(1,0,1))
	K<- (m-1)-J	
	while(gamL[d]>min.gam & lik.gam(gam=gamL, d=d, cv=cv, class.vec=class.vec, rhoh=rhoh, etah=etah, eta.vec=eta.vec, Ns=Ns, Zs=Zs) > z & J>0){gamL[d]<- max(gamL[d]-w,min.gam)
		J<- J-1}
		gamL[d]<-max(gamL[d],min.gam)
	while(gamR[d]<max.gam & lik.gam(gam=gamR, d=d,  cv=cv, class.vec=class.vec, rhoh=rhoh, etah=etah, eta.vec=eta.vec, Ns=Ns, Zs=Zs) > z & K>0){gamR[d]<- min(gamR[d]+w,max.gam)
		K<- K-1}
		gamR[d]<-min(gamR[d],max.gam)
	gam.star[d]<- runif(1,gamL[d],gamR[d])
	while(lik.gam(gam=gam.star, d=d, cv=cv, class.vec=class.vec, rhoh=rhoh, etah=etah, eta.vec=eta.vec, Ns=Ns, Zs=Zs) < z){
...
lik.rho<-function(rho, k, Ns, Zs, etah, class.vec, pm, cm, rho.s1, rho.s2){
	...
	return(sum(Ns[class.vec==k]*log(rho[k])) - rho[k]*sum(class.vec==k) + sum(Ns[Ns>0]*as.vector(cm[Ns>0,]%*%etah) *log(nu) ) - sum(Zs[Ns>0]*nu) + (rho.s1-1)*log(1-exp(-rho[k])) - rho.s2*rho[k]) }
...
lik.eta<-function(eta, k, rhoh, Ns, Zs, pm, cm, class.vec){
	nu<-sum(rhoh*eta*apply(pm,2,mean))
	return(sum(Ns[Ns>0 & class.vec==k]*eta[k]*log(Zs[Ns>0 & class.vec==k]+1e-100)) -sum(lgamma(Ns[Ns>0 & class.vec==k]*eta[k])) + sum(Ns[Ns>0]*as.vector(cm[Ns>0,]%*%eta)*log(nu)) -sum(Zs[Zs>0]*nu))}

slice.eta<-function(eta0, Ns, Zs, rhoh, class.vec, pm, cm){
	for(k in 1:3){
	...
	z<- lik.eta(eta=eta0, k=k,rhoh=rhoh,  Ns=Ns, Zs=Zs, pm=pm, cm=cm, class.vec=class.vec) - rexp(1,1)
	w<- 0.05
	m<- 10
	...
	J<- floor(m*runif(1,0,1))
	K<- (m-1)-J	
	while(etaL[k]>min.eta & lik.eta(eta=etaL, k=k,rhoh=rhoh,  Ns=Ns, Zs=Zs, pm=pm, cm=cm, class.vec=class.vec)> z & J>0){etaL[k]<- max(etaL[k]-w,min.eta)
		J<- J-1}
		etaL[k]<-max(etaL[k],min.eta)
	while(etaR[k]<max.eta & lik.eta(eta=etaR, k=k,rhoh=rhoh,  Ns=Ns, Zs=Zs, pm=pm, cm=cm, class.vec=class.vec) > z & K>0){etaR[k]<- min(etaR[k]+w,max.eta)
		K<- K-1}
		etaR[k]<-min(etaR[k],max.eta)
	eta.star[k]<- runif(1,etaL[k],etaR[k])
	while(lik.eta(eta=eta.star,k=k,rhoh=rhoh,  Ns=Ns, Zs=Zs, pm=pm, cm=cm, class.vec=class.vec) < z){
