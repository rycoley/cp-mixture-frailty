...
#Accompanies LC3-ds-IV.R and LC3-ds-source-IV.R 
...
rho.mat<-eta.mat<-p.class<-matrix(nrow=K, ncol=2)
rhohat<-c(-log(runif(1,0.6,0.9)), -log(runif(1,0.2,0.5)))
etahat<-c(runif(1,1.5,3.5), runif(1,3.5,5.5))

gam.mat<-matrix(nrow=K, ncol=D)
gamhat<-runif(D,-0.25,0.25)

cs<-vector(length=K)
cv <- c(runif(1,-0.75,0.75))
...
#		if(j%in%seq(1,A,ke)){print(j)}
		...
		lambdas[k.num]<-lam
		betas[k.num]<-bet
		for(k in 1:2){p.class[k.num,k]<-sum(class.vec==k)/n}
...
rho.res<-eta.res<-PC.res<-matrix(nrow=2,ncol=4)
for(k in 1:2){
...
c.res<-c(median(cs), cover(x=cs, true=0.25))

HD<-quantile(HD,p=c(0.5,0.025, 0.975))

print(c(seed,"completed"))

results<-list(bet.res=bet.res, lam.res=lam.res, rho.res=rho.res, eta.res=eta.res, PC.res=PC.res, gam.res=gam.res, c.res=c.res, HD=HD, seed=seed) 
return(results)}
