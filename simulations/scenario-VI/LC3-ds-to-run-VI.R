...
#Accompanies LC3-ds-VI.R and LC3-ds-source-I.R 
...
rhohat<-c( -log( runif(1,0.75,1)), -log(runif(1,0.50, 0.75)), -log(runif(1,0.1,0.5)) )
...
for(k in 1:3){
	rho.res[k,]<-c(median(rho.mat[,k]), cover(x=rho.mat[,k], true=rho.true))
	eta.res[k,]<-c(median(eta.mat[,k]), cover(x=eta.mat[,k], true=eta.true))
	PC.res[k,]<-c(median(p.class[,k]), cover(x=p.class[,k], true=1/3))}


gam.res<-matrix(nrow=D,ncol=4)
for(d in 1:D){gam.res[d,]<-c(median(gam.mat[,d]), cover(x=gam.mat[,d], true=0))}

c.res<-matrix(nrow=2,ncol=4)
for(k in 1:2){c.res[k,]<-c(median(cs[,k]), cover(x=cs[,k], true=0))}
